CREATE TABLE "app_ledger" (
	...
	CONSTRAINT "app_ledger_kind_check" CHECK ("app_ledger"."kind" in ('manual_fund', 'routed_income', 'reverse_payout', 'adjustment'))
);
...
ALTER TABLE "ledger" ADD CONSTRAINT "ledger_kind_check" CHECK ("ledger"."kind" in ('topup', 'charge', 'income', 'transfer_in', 'transfer_out', 'redeem', 'withdrawal', 'withdrawal_refund', 'app_fund', 'app_payout', 'adjustment'));